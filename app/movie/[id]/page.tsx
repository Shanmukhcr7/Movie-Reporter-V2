...
import { ThumbsUp, ThumbsDown, Share2, Calendar, Star, Info } from "lucide-react"
...
export default function MovieDetailsPage() {
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      ...
      <main className="container mx-auto px-4 -mt-[350px] relative z-20">
        <div className="flex flex-col md:flex-row gap-8">

          {/* Left Column: Poster & Actions */}
          <div className="w-full md:w-[300px] flex-shrink-0 space-y-6">
            <div className="relative aspect-[2/3] rounded-xl overflow-hidden shadow-2xl border-4 border-background/20 bg-muted">
              <Image
                src={movie.poster || movie.posterUrl || "/placeholder.svg"}
                alt={movie.title}
                fill
                        ...
                        <div className="flex items-start justify-between mb-2">
                          ...
                          <div className="flex items-center text-yellow-500 gap-1">
                            {Array.from({ length: 5 }).map((_, i) => (
                              <Star key={i} className={`h-3 w-3 ${i < (review.rating || 0) ? 'fill-current' : 'text-muted/30'}`} />
                            ))}
                          </div>
                        </div>
